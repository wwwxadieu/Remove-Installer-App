...
- Tự động (hoặc theo yêu cầu ở Settings) kiểm tra phiên bản mới trên GitHub Releases của
  ...
  found.
- Tuỳ chọn thêm mục **"Gỡ bằng ClearOut"** vào menu chuột phải của Windows
  ...
  Optional **"Uninstall with ClearOut"** entry on the right-click menu of any
...
Chạy trực tiếp bằng Visual Studio: mở `ClearOut.sln`, chọn cấu hình
...
Open `ClearOut.sln` in Visual Studio, select `Debug | x64`, and press F5. The
...
```powershell
dotnet publish src/ClearOut/ClearOut.csproj `
...
File thực thi (`ClearOut.exe`) và toàn bộ dependency sẽ nằm trong thư mục
`publish` — có thể copy sang máy khác chạy trực tiếp mà không cần cài .NET runtime.

The resulting `ClearOut.exe` (in the `publish` folder) is self-contained and
...
- Mục menu chuột phải (nếu bật) được ghi vào
  `HKEY_CURRENT_USER\Software\Classes\exefile\shell\ClearOutUninstall` và
  `...\lnkfile\shell\ClearOutUninstall` — chỉ ảnh hưởng tài khoản Windows hiện
  ...
  `HKEY_CURRENT_USER\Software\Classes\exefile\shell\ClearOutUninstall` and
  `...\lnkfile\shell\ClearOutUninstall` — current Windows account only, never
...
- Cùng toggle menu chuột phải ở trên cũng ghi thêm mục **"Gỡ nhanh..."** vào
  `...\shell\ClearOutQuickUninstall` (cả `exefile` và `lnkfile`) — chạy
  `ClearOut.exe --quick-uninstall "<path>"`, không mở cửa sổ chính, chỉ dùng
  ...
  `...\shell\ClearOutQuickUninstall` (both `exefile` and `lnkfile`) — it runs
  `ClearOut.exe --quick-uninstall "<path>"`, never opens the main window, and uses
...
- **File log lỗi**: app ghi mọi exception không bắt được (cả UI lẫn task nền), cùng
  các lần điều hướng thất bại, vào `%LOCALAPPDATA%\ClearOut\error.log`
  ...
  **Error log**: the app appends every unhandled exception (UI and background task
  alike), plus any failed navigation, to `%LOCALAPPDATA%\ClearOut\error.log`
